...
def process_item(data_item):
    question_item = data_item['question']
    candidates:list[str] = data_item['candidates']
    label = data_item['label']
    
    positive_item = candidates[label]
    
    question_results = labeler(question_item.get('text'),need_scopes=True)
    positive_result = labeler(positive_item.get('text'),need_scopes=True)
    
    question_item.setdefault('scopes', {})
    positive_item.setdefault('scopes', {})

    granularity_key = f'granularity_{labeler.granularity}'

    # 補進 granularity 範疇，如果還沒建立
    if granularity_key not in question_item['scopes']:
        question_item['scopes'][granularity_key] = question_results.get('word_scopes')

    if granularity_key not in positive_item['scopes']:
        positive_item['scopes'][granularity_key] = positive_result.get('word_scopes')
...
def main():
    ...
    for dir in os.listdir(Path(data_root_path,seg_tool)): 
        ...
        raw_data_path = Path(data_root_path,seg_tool,dir,f'{dir}_qa_hard_negatives_pretokenized.json')
        save_file_path = Path(data_root_path,seg_tool,dir,f'{dir}_qa_hard_negatives_labeled.json')
