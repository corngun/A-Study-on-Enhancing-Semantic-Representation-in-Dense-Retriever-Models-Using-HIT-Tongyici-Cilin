...
# class ScopeEnhancedSiameseEncoder(nn.Module):
#     def __init__(self,config,**kwargs):
#         super(ScopeEnhancedSiameseEncoder,self).__init__()
#         d_ebd,n_head,d_ff,dropout,number_of_layers = (
#             config['d_ebd'],
#             config['n_head'],
#             config['d_ff'],
#             config['dropout'],
...
#         self.encoder = Encoder(d_ebd,n_head,d_ff,dropout,number_of_layers)
...
#         q_out = self.encoder(q_seq_ebd,q_atten_mask) 
#         postive_out = self.encoder(postive_seq_ebd,postive_atten_mask) 
...
#         neg_out = self.encoder(flattened, flattened_mask) 
#         # neg_out_last_hidden_state = neg_out.last_hidden_state # (batch×3, seq_len, d_ebd)
#         # neg_setence_ebds = self.mean_pooling(neg_out_last_hidden_state,flattened_mask)  # (batch*3, d_ebd)
...
"""
scope_aware_pooling 針對 negative sample 處理的方式比較易理解但是訓練時間比較慢
"""
# class ScopeEnhancedDualEncoder(nn.Module):
#     def __init__(self,config,**kwargs):
#         super(ScopeEnhancedDualEncoder,self).__init__()
...
#         self.question_encoder = Encoder(d_ebd,n_head,d_ff,dropout,number_of_layers)
#         self.answer_encoder = Encoder(d_ebd,n_head,d_ff,dropout,number_of_layers)
...
#             q_setence_ebds = self.scope_aware_pooling(q_word_reprs,q_scope_logits,q_span_tables)
#             positive_sentence_ebds = self.scope_aware_pooling(positive_word_reprs,positive_scope_logits,positive_span_tables)
            
#             neg_out_last_hidden_state = neg_out.last_hidden_state # (batch×N, seq_len, d_ebd)
            
#             neg_word_reprs = []
#             neg_scope_logits = []
#             flattened_neg_span_tables = []
            
#             for i in range(B):
#                 for j in range(N):
#                     flattened_neg_span_tables.append(neg_span_tables[i][j]) #[B*N][global_neg_seq_len,word_spans]
            
#             for i in range(B*N):
#                 word_repr = self.average_by_spans(neg_out_last_hidden_state[i], flattened_neg_span_tables[i]) # (local_seq_len,d_ebd)
#                 scope_logit = self.scope_proj_head(word_repr) # (local_seq_len,num_scope)
#                 neg_word_reprs.append(word_repr) # [B*N][local_seq_len,d_ebd]
#                 neg_scope_logits.append(scope_logit)
            
#             neg_scope_logits = pad_sequence(neg_scope_logits, batch_first = True) #(batch*N,global_seq_len,num_scope)
#             # neg_word_reprs = pad_sequence(neg_word_reprs, batch_first = True)  #(batch*N,global_seq_len,d_ebd)
            
            
#             neg_sentence_ebds = self.scope_aware_pooling(
#                                                         neg_word_reprs, 
#                                                         neg_scope_logits, 
#                                                         flattened_neg_span_tables, 
#                                                         ) # (B*N, D)
            
#             neg_sentence_ebds = neg_sentence_ebds.view(B, N, -1)  # (batch, n, d_ebd)

                
#         else:   
#             # (batch,d_ebd)
#             q_setence_ebds = q_out.cls_out
#             positive_sentence_ebds = postive_out.cls_out
#             neg_setence_ebds = neg_out.cls_out
#             neg_sentence_ebds = neg_setence_ebds.view(B, N, -1) # (batch, 3, d_ebd)
...
#         """
#         Args:
#         ebds: Tensor (batch,seq_len,d_ebd)
#                 Encoder output representations 
#         attention_mask: Tensor (batch,seq_len)
#                 1 for real tokens, 0 for padding
                
#         Return:
#             Tensor (batch_size, hidden_dim): Mean pooled sentence embeddings
#         """
        
#         # (batch,seq_len) -> (batch,seq_len,1) -> (batch,seq_len,d_ebd)
#         expand_attention_mask = attention_mask.unsqueeze(dim=-1).expand(ebds.size())
        
#         # 避免除以0的現象，最低設為 1e-9
#         # (batch,seq_len,d_ebd) -> (batch,d_ebd) -> (batch,d_ebd)
